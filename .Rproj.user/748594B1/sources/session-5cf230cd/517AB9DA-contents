# ==============================================================================
# Parse eggNOG-mapper Results - UPDATED for hs, hm, ha naming
...
# Strain mapping for labels
strain_names <- c(
  "hs" = "Halobacterium salinarum (KBTZ01)",
  "hm" = "Halomicrobium kobeituziens (KBTZ05)",
  "ha" = "Haloarcula sp. (KBTZ06)"
)

# Load all three strains with new naming
cat("\nLoading eggNOG results...\n")
cat("-------------------------\n")

hs <- load_eggnog_annotations("hs")
hm <- load_eggnog_annotations("hm")
ha <- load_eggnog_annotations("ha")

# Combine all annotations
all_annotations <- rbind(hs, hm, ha, fill = TRUE)
...
for (strain in c("hs", "hm", "ha")) {
  count <- sum(all_annotations$strain_id == strain)
  cat(sprintf("  %s (%s): %d proteins\n", 
              strain, strain_names[strain], count))
}
cat("\n")
...
cat("Strain Summary:\n")
cat("  hs = Halobacterium salinarum (KBTZ01)\n")
cat("  hm = Halomicrobium kobeituziens (KBTZ05)\n")
cat("  ha = Haloarcula sp. (KBTZ06)\n\n")

cat("Next steps:\n")
cat("1. Review pathway presence/absence\n")
cat("2. Analyze enzyme differences between strains\n")
cat("3. Create visualizations (run 04_comparative_strain_analysis.R)\n")
